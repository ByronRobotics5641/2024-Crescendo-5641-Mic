...
public class TaxiShootAuto extends SequentialCommandGroup {
  /** Creates a new TaxiAuto. */


  public TaxiShootAuto(RevSwerve base, ShooterSubsystem shooter, IndexSubsystem index) {
    ...
    addCommands(
      ...
      new AutoSwerve(base, 0.2, 0, 0, false).withTimeout(.242),
      new AutoSwerve(base, 0, 0, 0, false).withTimeout(0.1)

    );

    // total auto runtime: 4 seconds, we can do things for 11 more seconds if needed.
